...
const generate = async (suppliedConfig) => {
    const config = (0, config_1.finaliseConfig)(suppliedConfig), log = config.progressListener === true ? console.log :
        ...
        config.debugListener || (() => void 0), { ts, customTypeSourceFiles } = await (0, tsOutput_1.tsForConfig)(config, debug), folderName = 'zapatos', schemaName = 'schema' + config.outExt, customFolderName = 'custom', customTypesIndexName = 'index' + config.outExt, customTypesIndexContent = (0, header_1.header)() + `
// this empty declaration appears to fix relative imports in other custom type files
declare module 'zapatos/custom' { }
`, folderTargetPath = path.join(config.outDir, folderName), schemaTargetPath = path.join(folderTargetPath, schemaName), customFolderTargetPath = path.join(folderTargetPath, customFolderName), customTypesIndexTargetPath = path.join(customFolderTargetPath, customTypesIndexName);
